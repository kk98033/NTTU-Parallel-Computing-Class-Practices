...
h_C[1023] ==   50.00
Kernel Execution =    0.88 ms
h_C[1023] ==   50.00
Shaered Memory Execution =    0.88 ms
...
__global__ void VecAddSharedMemory(float *A, float *B, float *C)
{
    int index = threadIdx.x;
...
int main()
{
    ...
    */
    ...
    cudaEventRecord(time1, 0);
    VecAddSharedMemory<<<dimgrid, dimblock>>>(d_A, d_B, d_C);
    cudaEventRecord(time2, 0);
    ...
    printf("h_C[%d] == %7.2f\n", N - 1, h_C[N - 1]);
    printf("Shaered Memory Execution = %7.2f ms\n", kernelExecutionTime);
