...
__global__ void VfCC20(uch *dstimg, uch *srcimg, ui hpxs, ui vpxs, ui LoopID)
{
    ui ThrPerBlk = blockDim.x;
    ui bid = (LoopID * 32768) + blockIdx.x;
    ui tid = threadIdx.x;
...
// 拷貝影像的PxCC20 kernel函式
__global__ void PxCC20(uch *dstimg, uch *srcimg, ui FS, ui LoopID)
{
    ui ThrPerBlk = blockDim.x;
    ui bid = (LoopID * 32768) + blockIdx.x;
    ui tid = threadIdx.x;
...
int main(int argc, char **argv)
{
    ...
    default:
        ...
        printf("\n\nExample: %s pic.bmp out.bmp v", ProgName);
        printf("\n\nExample: %s pic.bmp out.bmp c 128", ProgName);
        printf("\n\nv=vertical Rev, c=copy image\n\n");
        exit(EXIT_FAILURE);
        exit(EXIT_FAILURE);
    }
    if ((Rev != 'v') && (Rev != 'c'))
    {
        printf("Invalid Rev option '%c'. Must be 'v' or 'c'... \n", Rev);
        exit(EXIT_FAILURE);
    }
    ...
    BlkPerRow = (INFH + ThrPerBlk -1 ) / ThrPerBlk;    ui NumLoops, L;
    switch (Rev)
    {
    // 垂直翻轉
    case ' v':
        ...
        for (L = 0; L < NumLoops; L++)
            VfCC20<<<32768, ThrPerBlk>>>(gpudupimg, gpuimg, INFH, INFV, L);
        gpuresult = gpudupimg;
    ...
    case 'c':
        ...
        NumLoops = CEIL(NumBlocks, 32768);
        for (L = 0; L < NumLoops; L++)
            PxCC20<<<32768, ThrPerBlk>>>(gpudupimg, gpuimg, IMAGESIZE, L);
        gpuresult = gpudupimg;
        GPUDataTransfer = 2 * IMAGESIZE;
        break;
    }
